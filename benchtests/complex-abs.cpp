  ...
  struct Benchmark<Special>
  {
	...
	return {
	  0.25 * time_median([&] [[gnu::always_inline]] {
		   auto d = b;
		   auto r = std::abs(d); d_from_r(d, r);
		   r = std::abs(d); d_from_r(d, r);
		   r = std::abs(d); d_from_r(d, r);
		   r = std::abs(d); d_from_r(b, r);
		 }),
	  1./6. * time_median([&] [[gnu::always_inline]] {
		    auto d0 = data[0]; vir::fake_modify(d0); auto r0 = std::abs(d0);
		    auto d1 = data[1]; vir::fake_modify(d1); auto r1 = std::abs(d1);
		    auto d2 = data[2]; vir::fake_modify(d2); auto r2 = std::abs(d2);
		    auto d3 = data[3]; vir::fake_modify(d3); auto r3 = std::abs(d3);
		    auto d4 = data[4]; vir::fake_modify(d4); auto r4 = std::abs(d4);
		    auto d5 = data[5]; vir::fake_modify(d5); auto r5 = std::abs(d5);

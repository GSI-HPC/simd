...
namespace std::simd
{
  ...
  template <typename _Tp, typename _Abi>
    ...
    public:
      ...
      using mask_type = basic_mask<sizeof(conditional_t<is_void_v<_Tp>, int, _Tp>), _Abi>;
...
#undef _GLIBCXX_DELETE_SIMD
  ...
  template <typename _Tp, typename _Abi>
    ...
    public:
      ...
      //
      // Note that vec::vec is not a match, because it's not a contiguous range. Thus, if the
      ...
      template <std::ranges::contiguous_range _Rg, typename... _Flags>
        ...
        : _M_data(__data(std::load<basic_vec>(__range, __flags)))
        {
          static_assert(__detail::__loadstore_convertible_to<std::ranges::range_value_t<_Rg>,
                                                             value_type, _Flags...>);
      ...
      template <std::ranges::range _Rg, typename... _Flags>
        ...
        : _M_data{}
        {
          static_assert(false, "'ranges::to<basic_vec>()' requires a value-preserving conversion. "
                               "Call 'ranges::to<basic_vec>(flag_convert)' to allow all "
      ...
      _M_to_array() const noexcept
      {
        ...
        std::simd::unchecked_store(*this, __r);
        return __r;
      }
      ...
      template <std::integral _Up, typename _Ap>
        ...
        operator[](basic_vec<_Up, _Ap> const& __idx) const
        {
          ...
          using _Rp = resize_t<__detail::__simd_size_v<_Up, _Ap>, basic_vec>;
  ...
  template <__detail::__vectorizable _Tp, __detail::__vectorizable _Up,
    ...
    struct rebind<_Tp, basic_vec<_Up, _Abi>>
    {
      ...
      using type = basic_vec<_Tp, _Abi1>;
    };
  ...
  template <typename _Tp, typename _Abi, __detail::__vectorizable _Up>
    ...
    : std::integral_constant<size_t, alignof(rebind_t<_Up, basic_vec<_Tp, _Abi>>)>
    {};

  template <size_t _Bs, typename _Abi>
    struct alignment<basic_mask<_Bs, _Abi>, bool>
...
{
  template <typename _Tp, typename _Abi>
    struct __is_simd<std::simd::basic_vec<_Tp, _Abi>>
    : is_default_constructible<std::simd::basic_vec<_Tp, _Abi>>
    {};
}
